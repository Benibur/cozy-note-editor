...
// HACKED: from now, headers are replaced with <p></p>
var _DoHeaders = function(text) {
	...
	//
	text = text.replace(/^(.+)[ \t]*\n=+[ \t]*\n+/gm,
		function(wholeMatch,m1){return hashBlock(make_tag("<p class='TH'>", m1)+ _RunSpanGamut(m1) + "</p>");});

	text = text.replace(/^(.+)[ \t]*\n-+[ \t]*\n+/gm,
		function(matchFound,m1){return hashBlock(make_tag("<p class='TH'>", m1)+ _RunSpanGamut(m1) + "</p>");});
	...
	*/
	...
	text = text.replace(/^(\#{1,6})[ \t]*(.+?)[ \t]*\#*\n+/gm,
		function(wholeMatch,m1,m2) {
			return hashBlock(make_tag("<p class='TH'>", m2) + _RunSpanGamut(m2) + "</p>");
		});
